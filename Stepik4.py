...
def display_hangman(tries):
    ...
    return stages[tries]
def play():
    allletters = 'ЁЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮ'
    print('Давайте играть в угадайку слов! Вводите русские буквы!')
    tries = 6
    qword = choice(word_rus).upper()
    word_completion = '_' * len(qword)
    guessed_letters = []
    ...
    while tries > 0:
        ...
        if letter in guessed_letters:
            print("Эта буква уже была, введите другую")
            continue
        elif letter in qword:
            for i in range(len(qword)):
                if letter == qword[i]:
                    word_completion = word_completion[:i] + letter + word_completion[i+1:]
            guessed_letters.append(letter)
        elif letter in allletters and letter not in qword:
            tries -= 1
            guessed_letters.append(letter)
        else:
            print("Введена некорректная буква, попробуйте снова")
            continue
        print(word_completion)
        print(display_hangman(tries))
        print('Использованные буквы: ',*guessed_letters,'\n')
        if '_' not in word_completion:
            print("Поздравляем вы выиграли!")
            break
    else:
        print("Вы проиграли, слово было: ", qword)

re = 'д'
while re == 'д':
    play()
    re = input('Хотите сыграть еще? д/н\n')
else:
    exit()
